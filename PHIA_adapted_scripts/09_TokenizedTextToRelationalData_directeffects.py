...
def extendAssociationTypesToNeighboringAdjectNegatives(dataframe):
    ...
    for turn in [1,2,3]:
        ...
        dataframe.loc[y, 'Tag'] = 'I-AssocType'
        dataframe.loc[g, 'Tag'] = 'I-AssocType'
...
def LabelAssocTypeWords(labeled_data):
    ...
    labeled_data.loc[x, 'Tag'] = 'I-AssocType'
    return labeled_data
...
def FindIndexesPerLabel_IO(labeled_data, sentence_idx_range):
    ...
    AT_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-AssocType")[0])
    ET_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-ExposureType")[0])
    HE_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-HealthEffects")[0])
    SG_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-StudyGroup")[0])
    SE_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-StudyEnvironment")[0])
    ST_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-StudyType")[0])
    MO_indx = list(np.where(labeled_data.loc[sentence_idx_range, 'Tag'] == "I-Moderator")[0])
    return AT_indx, ET_indx, HE_indx, SG_indx, SE_indx,ST_indx, MO_indx
...
############ Abbreviation list
## ST = study, HE = HealthEffects, ET = Exposure Type, AT = Association Type, SG = Study Group, SE = StudyEnvironment, ST = StudyType, MO = Moderator
...
datafolder = r"C:/Users/4209416/OneDrive - Universiteit Utrecht/Desktop/ETAIN/PHIA_framework/Automated-KG/newCrossRef/Direct_Effects/final_kg"
os.chdir(datafolder)
if manual_label == True:
    listOfFiles = os.listdir(path=os.path.join(os.getcwd(), "Manually_labelled"))
else:
    listOfFiles = os.listdir(path=os.path.join(os.getcwd(), "Predict_labelled_cleaned"))
    print(listOfFiles)

instance_ST, instance_HE, instance_ET, instance_AT, instance_SG, instance_MO, instance_SE, instance_STyp, sentenceid, fullsentence, AT_POS, ET_POS, HE_POS, sentence_POS = [], [], [], [], [], [], [], [], [], [], [], [], [], []
...
for file in listOfFiles:
    ...
    if manual_label == True:
        ...
        labeled_data = pd.read_csv(os.path.join(os.getcwd(), ("Predict_labelled_cleaned/" + file)), encoding="latin1")
    ...
    labeled_data = extendVariableNamesToNeighboringAdjectNouns(labeled_data, ["I-ExposureType", "I-HealthEffects", "I-Moderator"])
    ...
    labeled_data = extendSpecificTagsToAllEqualWordSeq(labeled_data, "I-ExposureType")
    ...
    for sentence in sentences:
        ...
        if any(labeled_data['Tag'].iloc[sentence_idx_range] != 'O'):
            if mode == "IO":
                ...
                AT_indx, ET_indx, HE_indx, SG_indx, SE_indx,ST_indx, MO_indx= FindIndexesPerLabel_IO(labeled_data, sentence_idx_range)
                instance_AT.append(" ; ".join(IO_sequence_tracing(indx_list= AT_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                instance_ET.append(" ; ".join(IO_sequence_tracing(indx_list=ET_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                instance_HE.append(" ; ".join(IO_sequence_tracing(indx_list=HE_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                instance_SG.append(" ; ".join(IO_sequence_tracing(indx_list=SG_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                instance_SE.append(" ; ".join(IO_sequence_tracing(indx_list=SE_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                instance_STyp.append(" ; ".join(IO_sequence_tracing(indx_list=ST_indx, Words_data=labeled_data['Word'].iloc[sentence_idx_range])))
                ...
                ET_POS.append(" ; ".join(sent_POS.iloc[ET_indx]))
                HE_POS.append(" ; ".join(sent_POS.iloc[HE_indx]))
...
print("Number of instances: ", len(instance_AT))
print("Number of sentences: ", len(sentenceid))
print("Number of full sentences: ", len(fullsentence))
print("Number of AT: ", len(instance_AT))
print("Number of ET: ", len(instance_ET))
print("Number of HE: ", len(instance_HE))
print("Number of SG: ", len(instance_SG))
print("Number of SE: ", len(instance_SE))
print("Number of ST: ", len(instance_STyp))
print("Number of MO: ", len(instance_MO))
print("Number of sentence_POS: ", len(sentence_POS))
print("Number of AT_POS: ", len(AT_POS))
print("Number of ET_POS: ", len(ET_POS))
print("Number of HE_POS: ", len(HE_POS))


# make dataframe
Evidence_instances_df = pd.DataFrame({'DOI': instance_ST, 'Sentence': sentenceid, 'HealthEffects': instance_HE, 
                                      'ExposureType': instance_ET, 'AssociationType': instance_AT, 
                                      'StudyGroup': instance_SG, 'StudyEnvironment': instance_SE, 'StudyType': instance_STyp,
                                      'Moderator': instance_MO ,  
                                      'Fullsentence': fullsentence, 'HE_POS': HE_POS, 'ET_POS': ET_POS, 
                                      'AT_POS': AT_POS, 'sentence_POS': sentence_POS })
...
complete_evidence_Instances = Evidence_instances_df.iloc[list(np.where((Evidence_instances_df['HealthEffects'] != "") & (Evidence_instances_df['AssociationType'] != ""))[0])]
...
if manual_label == False:
    unique_ET, freq_ET = UniqueLabelsFreq(complete_evidence_Instances['ExposureType'])
    pd.DataFrame({'ExposureType': unique_ET, 'Freq': freq_ET}).to_csv(os.path.join(os.getcwd(), ("unique_ExposureType.csv")), index=False)

    unique_HE, freq_HE = UniqueLabelsFreq(complete_evidence_Instances['HealthEffects'])
    pd.DataFrame({'HealthEffects': unique_HE, 'Freq': freq_HE}).to_csv(os.path.join(os.getcwd(), ("unique_HealthEffects.csv")), index=False)
